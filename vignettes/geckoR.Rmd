...
vignette: >
  %\VignetteIndexEntry{Using screenR}
...
This step can be performed using any tool of your choice, but *screenR* provides an option to do this via the *Rsubread* package, using the function `screenR.align()`.
...
screenR.align(path = "/g/korbel/shared/projects/crispr_screens/soft_agar/mcf10a/raw_data/", 
...
An example of providing them to the `screenR.align()` function might look something like the code below. *NOTE: this actual example will perform badly, as it will align reads to the wrong guide library.*
...
After alignment we need to count how many reads aligned to each guide sequence.  This is simlar to using *samtools idxstat* if you're using an external program.  *screenR* performs this using the function `screenR.count()`.  Many of the arguments are the same as seen above, but not that this time `path` is the location of the aligned BAM files, and `outputDir` is the location where the table of counts should be created.
...
screenR.count(path = "/g/korbel/shared/projects/crispr_screens/soft_agar/mcf10a/aligned", 
              sampleSheet = "SampleSheet.txt", 
...
Once you've performed the alignement and guide counting we're ready to read the count data.  This is performed using the function `screenR.readCounts()`, which takes two arguments.  One of these is the name of the sample sheet file, and the other is the path where the sample sheet and read count files can be found.

```{r, readData, message=FALSE, cache=TRUE}
guideCounts <- screenR.readCounts(sampleSheet = "SampleSheet.txt", 
...
guideCounts <- screenR.normalizeBetweenSamples(guideCounts)
```
...
```{r, qc_plots2, fig.height=5, fig.width=12.5, dev='png', message=FALSE}
screenR.guideDistribution(guideCounts)
```

If you have replicates of each condition (and you should have replicates) we can compare the counts of control guides using the function `screenR.compareReplicates()`.  By default this will try to compare the counts for all guides across the replicates, not just the controls.  This can be very slow, so we can use the argument `controlString` to specify a pattern in the guide names that we should use to select the controls.  For the GeCKO libraries the control guides all contain the word 'control', but this be different for different CRIPR libraries.   If you have more than two replicates this function will show each compared to all the rest e.g. in the case of 3 replicates: rep1 vs rep2, rep1 vs rep3 & rep2 vs rep3.

```{r, qc_plots, fig.height=6, fig.width=10, dev='png', message=FALSE}
screenR.compareReplicates(guideCounts, controlString = "control")
```
...
The model can be fitted using the function `screenR.fitModel()`.  This calculates

```{r, eval=FALSE}
topTable <- screenR.fitModel(guideCounts)
```
...
```{r}
screenR.volcanoPlot(topTable, p.thresh = 0.05, fc.thresh = 1)
```
